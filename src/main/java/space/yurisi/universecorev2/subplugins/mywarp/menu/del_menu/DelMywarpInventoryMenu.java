package space.yurisi.universecorev2.subplugins.mywarp.menu.del_menu;
...
public class DelMywarpInventoryMenu implements BaseMenu {

    private UniverseCoreAPIConnector connector;

    public DelMywarpInventoryMenu(UniverseCoreAPIConnector connector){
    ...
    public void sendMenu(Player player) {
        try{
            List<Item> items = this.connector.getMywarpList(player).stream()
                    .map(mywarp -> new DelMenuItem(connector, mywarp))
            ...
            player.sendMessage(MessageHelper.getErrorMessage("ユーザーデータが存在しないようです。管理者に報告してください。 コード MW1"));
        } catch (MywarpNotFoundException e) {
            player.sendMessage(MessageHelper.getErrorMessage("ワープポイントが見つかりませんでした。コード MW2"));
        }
    }
}
